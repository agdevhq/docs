...
-   **Events**: During a run, the agent emits a series of `events` that provide a real-time log of its planning, visible narration, provider reasoning traces, and tool activity. The main event types are `planning`, `narration`, `reasoning`, and `tool_call`, so you can distinguish between user-facing progress updates and provider-native reasoning metadata.
-   **Result**: Some completed runs produce `resultData`: either a text `document` or a structured `json` object, depending on the agent's configuration. Action-only agents can complete successfully without a result; in that case `resultType` and `resultData` are `null`, and the event log shows the tools the agent used to achieve its goal.
