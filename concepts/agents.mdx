...
-   **Drives**: Agents can be bound to one or more drives, which provide workspace access through MCP tools. When drives are attached, the agent automatically receives tools to search and interact with the workspace content. **Note**: An agent must have at least one tool or drive binding configured. Drive bindings provide implicit tools, so an agent with only drives (and no explicit tools) is valid.
...
-   **Output Configuration**: You can configure an agent to return a freeform `document`, a structured `json` object that conforms to a specific schema, or no result at all. Agents with no result complete once they have achieved their goal through tools and stop naturally.
